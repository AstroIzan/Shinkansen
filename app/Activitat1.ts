// Control de errores
    let first_time:boolean = true;              // Variable [bool] para determinar si es la primera vez que se entra al codigo
    let first_click:boolean = true;             // Variable [bool] para determinar si es la primera vez que se mueve el tren
    let avisao_izq:boolean = false;             // Variable [bool] para determinar si se ha desplegado ya el modal izquierdo
    let avisao_der:boolean = false;             // Variable [bool] para determinar si se ha desplegado ya el modal derechoç
    let mobile:boolean = false;                 // Variable [bool] para determinar si se ha escogido movil o no
    let first_time_vagon:any = [];              // Variable [Array] para determinar si es la primera vez que se entra en el vagon
    let a:number = 0;                           // Variable [int] para determinar la posicion del tren

// Modal's
    let maft:any|null = document.getElementById("modal_error_final_tren");      // Variable [HTMLElement] que almacena el modal donde mostrar el error del tren
    maft.style.display = "none";                                                // Aplicamos un display none para evitar que se muestre
    let met:any|null = document.getElementById("modal_error_tipo");             // Variable [HTMLElement] que almacena el modal donde mostrar el error del tipo de dispositivo
    let set_tipo:any|null = document.getElementById("set_tipo");                // Variable [HTMLElement] que almacena el tipo donde insertar datos del tipo
    let set_texto:any|null = document.getElementById("set_texto");              // Variable [HTMLElement] que almacena el texto donde insertar datos del tipo
    met.style.display = "none";                                                 // Aplicamos un display none para evitar que se muestre

// Login Section
    let cUser:any|null = document.getElementById("cUser");                      // Variable [HTMLElement] que almacena el valor del input de usuario a la hora de iniciar sesion
    let mUser:any|null = document.getElementById("mUser");                      // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario                    
    let cPass:any|null = document.getElementById("cPass");                      // Variable [HTMLElement] que almacena el valor del input de contraseña a la hora de iniciar sesion
    let mPass:any|null = document.getElementById("mPass");                      // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario
    let mLogin:any|null = document.getElementById("mLogin");                    // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario
...
// Sign Up Section
    let cUserSign:any|null = document.getElementById("cUser2");                 // Variable [HTMLElement] que almacena el valor del input de usuario a la hora de iniciar sesion
    let mUserSign:any|null = document.getElementById("mUser2");                 // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario   
    let cPass1Sign:any|null = document.getElementById("cPass2");                // Variable [HTMLElement] que almacena el valor del input de usuario a la hora de iniciar sesion
    let mPass1Sign:any|null = document.getElementById("mPass2");                // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario  
    let cPass2Sign:any|null = document.getElementById("cPass3");                // Variable [HTMLElement] que almacena el valor del input de usuario a la hora de iniciar sesion
    let mPass2Sign:any|null = document.getElementById("mPass3");                // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario  
    let mPassDont:any|null = document.getElementById("mPassDont");              // Variable [HTMLElement] que almacena el span necesario para notificar de algun fallo al usuario  
...
// Train Section
    let posicion:any = document.getElementById("posicion");                     // Variable [HTMLElement] que almacena la posicion del tren
    let clase_vagon:any|null = document.getElementById("clase_vagon");          // Variable [HTMLElement] que se utiliza para mostrar en pantalla la clase del vagon
    let train_move:any = document.getElementById("train_move");                 // Variable [HTMLElement] que almacena el tren
    let flecha_izq:any|null = document.getElementById("flecha_izq");            // Variable [HTMLElement] que nos permite modificar el aspecto de la flecha
    let flecha_der:any|null = document.getElementById("flecha_der");            // Variable [HTMLElement] que nos permite modificar el aspecto de la flecha
    let div_asientos:any|null = document.getElementById("asientos");            // Variable [HTMLElement] donde se mostraran los asientos
    let vagones:any|null = document.getElementsByClassName("vagon");            // Variable [Array] donde almacenaré los objetos del array
    let msa:any|null = document.getElementsByName("insert_asientos");           // Variable [HTMLElement] donde se insertan los asientos
...
// Asientos Section
    ...
    let asientos:any;                                                           // Variable [HTMLEelements] donde almacenar todos los asientos del vagon
    let asientos_reservados:any = [];
    let asientos_comprados:any = [];
    let precio:any|null = document.getElementById("precio");                    // Variable [HTMLElement] donde almacenar el importe total
...
// SECTIONS
    let s_login:any|null = document.getElementById("login");                    // Variable [HTMLElement] que almacena la seccion de login
    let s_signup:any|null = document.getElementById("signup");                  // Variable [HTMLElement] que almacena la seccion de signup
    let s_train:any|null = document.getElementById("train");                    // Variable [HTMLElement] que almacena la seccion de train
    let s_asientos:any|null = document.getElementById("asientos");              // Variable [HTMLElement] que almacena la seccion de asientos
...
// Metodo del login
    ...
    // En caso que el usuario no haya introducido el username se le notifica
        if ( user == "" ) { mUser.style.display = "block"; } else { mUser.style.display = "none"; }
    // En caso que el usuario no haya introducido la contraseña se le notifica
        if ( pass == "" ) { mPass.style.display = "block"; } else { mPass.style.display = "none"; }
...
// Metodo del signup
    ...
    // En caso que el usuario no haya introducido el username se le notifica
        if ( user == "" ) { mUserSign.style.display = "block"; } else { mUserSign.style.display = "none"; }
    // En caso que el usuario no haya introducido la primera contraseña se le notifica
        if ( pass1 == "" ) { mPass1Sign.style.display = "block"; } else { mPass1Sign.style.display = "none"; }
    // En caso que haya introducido todos los datos seguimos
        if ( pass1 != pass2 ) { mPassDont.style.display = "block"; } else { mPassDont.style.display = "none"; }
    // En caso que todo este correcto se continua con el programa, si las contraseñas no coinciden se muestra el mensaje de error
        ...
        // Se cambia el display a la pagina de login
            toLogin();
        } else { mPass2Sign.style.display = "block"; }
    }
...
// Metodos para obtener y setear las diferentes variables disponibles
    ...
    // Variable avisao_izq [ Variable que indica si se ha mostrado el modal de aviso de que no se puede ir mas a la izquierda ]
        ...
        // Metodo set
            function setAvisoIzq( setFunc:boolean ) { avisao_izq = setFunc; }
    ...
    // Variable de movile [ Variable que nos permite saber si hemos escogido movil ]
        ...
        // Metodo set
            function setMobile( setFunc:boolean ) { 
                if ( setFunc == true ) {
                    mobile = true;
                } else {
                    mobile = false;
...
// Metodos para moverse por las diferentes secciones
    // Metodo para cerrar todos los modal de una
        ...
        // Metodo set del modal de tipo
            function setModalTipo( setFunc:boolean ) {
                if ( setFunc == true ) {
...
// Metodo de la animacion del texto que aparece en los trenes
    function textAnimation() {
        setInterval(function() {
            let move_cartel:any = document.getElementsByClassName("move_cartel");
...
// A PARTIR DE AQUI TIENES QUE MIRARTE SI O SI EL README XD
    function setVagones() {
        ...
        for (let i = 0; i < vagones.length; i++) {
            ...
            if ( i == 0 ) {
                t_insert = "";
                for (let x = 0; x < 4; x++) {
                    if ( x == 2 ) { t_insert += "<div class='separador'></div>" }
    ...
    function selectVagon(n_vagon:number) {
        toAsientos();
    ...
    function cerrar_modal_asientos(n_vagon:number) {
        toTrain();
        for (let i = 0; i < vagones.length; i++) {     
            clases[i].style.display = "none";   
        } 
    ...
    function reservar(n_asiento:number) {
    ...
    function comprar(n_asiento:number) {
        for (let i = 0; i < asientos.length; i++) {
    ...
    function guardar(n_asiento:number) {
        let save_asientos:any = [];
        for (let i = 0; i < asientos.length; i++) {
